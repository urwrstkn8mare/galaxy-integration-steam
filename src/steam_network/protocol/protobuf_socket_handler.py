...
class ProtocolParser:
    ...
    async def on_logon_success(self, interval: int):
    ...
    #send off heartbeats so steam doesn't kick us off. 
    async def _heartbeat(self, interval: int):
        message = CMsgClientHeartBeat()
        while True:
            await asyncio.sleep(interval)
            await self._send_no_wait(message, EMsg.ClientHeartBeat, next(self._job_id_iterator))